...
module Robot
  ...
  if loader.goto(url) then
    ...
    page=parser.page
    menu_nodes=parser.parse_by_record(page,menu_record)
    menu.add(menu_nodes)
    ...
    ali_menu.tree.each do |item|
      ...
      d_set.head_save_to_file(path,item[:title])
      blocks.each do |block|
        d_set.values=parser.by_data(block,d_set)
        d_set.save_to_file(path,item[:title])
      end
